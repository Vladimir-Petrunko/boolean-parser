...
public class Or extends Operation {
    ...
    public boolean apply(Boolean... args) {
        return Arrays.stream(args).reduce(false, Boolean::logicalOr);
    }
    ...
    private static Expression conjunctiveTwo(Expression left, Expression right) {
        ...
        if (left instanceof Literal) {
            ...
            newArgs = Utils.removeDuplicates(newArgs, And.class);
    ...
    public Expression conjunctiveNormalForm() {
        ...
        if (dis instanceof Or) {
            Or or = (Or) dis;
            Expression current = conjunctiveTwo(or.getArg(0), or.getArg(1));
            for (int i = 2; i < or.argCount(); i++) {
                current = conjunctiveTwo(current, or.getArg(i));
            }
            return current;
        } else {
            return dis;
